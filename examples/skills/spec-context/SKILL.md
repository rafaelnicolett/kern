...
`query_ontological`). Its whole value only shows up if you actually call
it before writing code, not after.

## When starting work on a task

1. **Resolve the task's name to its real id first.** Most of kern's tools
   take a real `entity_id` (a UUID), not a human-readable name —
   `query_by_concept` is how you get it, and it also returns the entity's
   direct relations in the same call.
...
2. **Walk the real dependency graph before touching code**, using that id
   with `get_related_entities` at depth 2 or more — a task's `implements`
...
3. **Prefer this two-step flow over `query_ontological` once the project
   has more than a handful of similar files.** This isn't a style
   preference — it's a real, verified finding: `query_ontological`'s
   semantic router gets measurably less reliable as more structurally
   similar files (several tasks with near-identical frontmatter, say)
   enter the corpus, and can return a plausible-looking answer about the
   *wrong* entity instead of erroring (see [`examples/README.md`](../README.md#5-a-real-limitation-relation-deduplication-and-routing-at-scale)
...
4. **Cite evidence, don't paraphrase from memory.** If a specific relation
   matters to a decision you're about to make, pull its real path and
   evidence with `explain_relation`, using both entities' real ids —
   rather than describing the relation from the frontmatter alone.
...
kern's ontology only knows what's actually in the indexed corpus — it has
no opinion on code outside the specs/plans/tasks folder, and its relation
...
are — they walk the real stored graph, not a similarity search.
